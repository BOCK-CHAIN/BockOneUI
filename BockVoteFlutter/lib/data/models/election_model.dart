...
class Election extends Equatable {
  ...
  factory Election.fromJson(Map<String, dynamic> json) {
    ...
    final startDate = _parseDateOrNull(json['startDate']) ??
        _parseUnixDateOrNull(json['startTime']) ??
        now;
    final endDate = _parseDateOrNull(json['endDate']) ??
        _parseUnixDateOrNull(json['endTime']) ??
        now;

    final dynamicCandidates = json['candidates'];
    final candidateIdsFromCandidates = dynamicCandidates is List
        ? dynamicCandidates
            .whereType<Map>()
    ...
    final statusString = (json['status'] ?? 'draft').toString();
    ...
    return Election(
      ...
      startDate: startDate,
      endDate: endDate,
      status: ElectionStatus.fromString(_normalizeElectionStatus(statusString)),
      type: ElectionType.fromString(json['type'] as String? ?? 'general'),
      imageUrl: json['imageUrl'] as String?,
      candidateIds: List<String>.from(
        (json['candidateIds'] as List?) ?? candidateIdsFromCandidates,
      ),
      settings: json['settings'] as Map<String, dynamic>?,
      createdAt: _parseDateOrNull(json['createdAt']) ??
          _parseUnixDateOrNull(json['timestamp']) ??
          now,
      updatedAt: _parseDateOrNull(json['updatedAt']) ??
          _parseUnixDateOrNull(json['timestamp']) ??
          now,
      createdBy: (json['createdBy'] ?? json['adminKey'] ?? 'system').toString(),
    );
  }
...
DateTime? _parseUnixDateOrNull(dynamic value) {
  if (value is int) {
...
String _normalizeElectionStatus(String status) {
  switch (status) {
    case 'pending':
      return 'scheduled';
    case 'ended':
