...
class _VoterRegistrationScreenState extends State<VoterRegistrationScreen> {
  ...
  Future<void> _register() async {
    ...
    try {
      ...
      final existingKey = prefs.getString('bockvote_private_key_seed');
      final privateKey = existingKey ??
          'voter-${_idNumberController.text.trim()}-${DateTime.now().microsecondsSinceEpoch}';
      await prefs.setString('bockvote_private_key_seed', privateKey);

      final voterId = _idNumberController.text.trim();
      await dio.post(
        '/voting/register/voter',
        data: {
          'voterId': voterId,
          'ipfsDocHash': 'doc-$voterId',
        },
        options: Options(
          headers: {
      ...
      // Best-effort auto-approval. Registration should still succeed if this
      // backend rejects admin approval in local/dev mode.
      try {
        await dio.post(
          '/voting/approve/voter',
          data: {
            'id': voterId,
            'approve': true,
          },
          options: Options(
            headers: {
              'X-Admin-Key': 'bockvote-admin',
            },
          ),
        );
      } on DioException {
        // Ignore approval errors and continue with pending voter state.
      }
